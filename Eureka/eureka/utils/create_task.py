...
def create_task(root_dir, task, env_name, suffix):
  ...
  input_file = f"{root_dir}/cfg/train/{task}Alg.yaml"
  output_file = f"{root_dir}/cfg/train/{task}{suffix}Alg.yaml"

  with open(input_file, 'r') as yamlfile:
    data = yaml.safe_load(yamlfile)

  # Modify the "name" field
  data['params']['config']['name'] = data['params']['config']['name'].replace(
      task, f'{task}{suffix}')
