...
def get_high_level_pref_gpt(state: Dict, prev_option: int, option: int) -> Tuple[float, Dict[str, float]]:
    ...
    # Constants to aid with task recognition
    OPTION_GO_TO_CIRCLE = rw4t_utils.RW4T_HL_Actions_EZ.go_to_circle.value
    OPTION_DELIVER_CIRCLE = rw4t_utils.RW4T_HL_Actions_EZ.deliver_circle.value
    OPTION_GO_TO_SQUARE = rw4t_utils.RW4T_HL_Actions_EZ.go_to_square.value
    OPTION_DELIVER_SQUARE = rw4t_utils.RW4T_HL_Actions_EZ.deliver_square.value

    # Extract relevant information from state
    agent_pos = state['pos']
    holding = state['holding']
    object_map = state['map']

    # Determine last delivered object type
    last_delivered_type = None
    if prev_option == OPTION_DELIVER_CIRCLE:
        last_delivered_type = rw4t_utils.Holding_Obj.circle.value
    elif prev_option == OPTION_DELIVER_SQUARE:
        last_delivered_type = rw4t_utils.Holding_Obj.square.value

    # Count remaining objects of each type in the environment
    remaining_circles = np.sum(object_map == rw4t_utils.RW4T_State.circle.value)
    remaining_squares = np.sum(object_map == rw4t_utils.RW4T_State.square.value)

    # Reward initialization
    preference_reward = 0.0
    reward_components = {
        'match_previous_delivery': 0.0
    }

    # Encourage same-type pickup if possible
    if last_delivered_type is not None:
        if last_delivered_type == rw4t_utils.Holding_Obj.circle.value and remaining_circles > 0:
            if option == OPTION_GO_TO_CIRCLE:
                preference_reward += 1.0
                reward_components['match_previous_delivery'] = 1.0
        elif last_delivered_type == rw4t_utils.Holding_Obj.square.value and remaining_squares > 0:
            if option == OPTION_GO_TO_SQUARE:
                preference_reward += 1.0
                reward_components['match_previous_delivery'] = 1.0
    # Diversify if the last type is fully delivered
    elif last_delivered_type == rw4t_utils.Holding_Obj.circle.value and remaining_circles == 0:
        if option == OPTION_GO_TO_SQUARE:
            preference_reward += 1.0
            reward_components['match_previous_delivery'] = 1.0
    elif last_delivered_type == rw4t_utils.Holding_Obj.square.value and remaining_squares == 0:
