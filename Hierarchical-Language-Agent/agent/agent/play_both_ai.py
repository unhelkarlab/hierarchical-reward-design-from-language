...
from agent.both_ai_gameplay import GamePlay
...
def parse_arguments():
  ...
  parser.add_argument("--agent_1",
                      type=str,
                      choices=['HLA', 'SMOA', 'FMOA', 'NEA'],
                      default='HLA')
  parser.add_argument("--agent_2",
                      type=str,
                      choices=['HLA', 'SMOA', 'FMOA', 'NEA'],
                      default='HLA')
...
def init_env_replay(map_name, agent_1_name, agent_2_name):
  map_set = MapSetting(**MAP_SETTINGS[map_name])
  agent_set_1 = AgentSetting(agent_1_name,
                             speed=2.5 if map_name != 'quick' else 3.5)
  agent_set_2 = AgentSetting(agent_2_name,
                             speed=2.5 if map_name != 'quick' else 3.5)
  ...
  game = GamePlay(env, replay, agent_set_1, agent_set_2)

  replay['set_map'] = deepcopy(map_set)
  replay['set_agent'] = deepcopy(agent_set_1)
...
if __name__ == '__main__':
  ...
  # initialize replay
  game, env, replay = init_env_replay(arglist.map, arglist.agent_1,
                                      arglist.agent_2)
  ...
  replay.save(
      repdir /
      f'{arglist.map}-{arglist.agent}-{datetime.now().strftime("%Y%m%d_%H%M%S")}.rep'
  )
