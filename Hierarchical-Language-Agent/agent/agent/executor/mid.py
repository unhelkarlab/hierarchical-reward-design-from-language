  ...
  def __call__(self, env: EnvState):
    ...
    while True:
      if self._stage == 1:
        ...
        if not env.check_pos_by_obj_gs(
            pos=self._pos, obj=self._task, gs='Cutboard'):
          self._stage = 1
          continue
        ...
        if not env.check_pos_by_obj_gs(self._pos, obj=self._task,
                                       gs='Cutboard'):
  ...
  def __call__(self, env: EnvState):
    while True:
      if self.stage == 1:
        ...
        if not env.check_pos_by_obj_gs(
            self.pos, obj='Fire', gs=None, inner=True):
          self.stage = 1
          continue
...
class MTDoOnce(MidTask):
  ...
  def can_begin(self, env: EnvState) -> bool:
    for task in self.AvailTask:
      ...
      # match
      pos = env.navigate_pos_by_obj_gs(obj=task['prep']['targ']['obj'],
                                       gs=task['prep']['targ']['gs'])
      if pos is None:
        continue
    ...
    while True:
      if self._stage == 1:
        ...
        if not match_any(fname(env.hold), self._task['prep']['hold']):
          return self.Failed, (
              0, 0), f"{fname(env.hold)} is not {self.Desc['adj']}"
        self._pos = pos
        self._stage = 2
      elif self._stage == 2:
        if not env.check_pos_by_obj_gs(self._pos,
                                       obj=self._task['prep']['targ']['obj'],
                                       gs=self._task['prep']['targ']['gs']):
          self._stage = 1
          continue
        status, move = LTApproach(self._pos)(env)
        ...
        if match_any(fname(env.hold), self._task['after']['hold']) \
          ...
          return self.Failed, (
              0, 0), f"Can't carry out {self.Desc['v']} near {self.PREP_GS}"
  ...
  AvailTask = [
      {  # hold something, put on empty counter
          "prep": {
              ...
              "hold":
              FRESH_FOOD + ASSEMBLE_CHOPPED_FOOD + ASSEMBLE_CHOPPED_PLATE_FOOD +
              ASSEMBLE_COOKED_PLATE_FOOD + ASSEMBLE_CHARRED_PLATE_FOOD +
              ['FireExtinguisher', 'Plate']
          },
          "after": {
              "targ": {
                  "obj":
                  FRESH_FOOD + ASSEMBLE_CHOPPED_FOOD +
                  ASSEMBLE_CHOPPED_PLATE_FOOD + ASSEMBLE_COOKED_PLATE_FOOD +
                  ASSEMBLE_CHARRED_PLATE_FOOD + ['FireExtinguisher', 'Plate'],
                  "gs":
                  PUTTABLE_GRIDSQUARES
              },
              "hold": "Nothing"
          }
      },
...
class MTPick(MTDoOnce):
  ...
  def __init__(self,
               obj: Union[str, List[str]] = None,
               gs: Union[str, List[str]] = None):
    super().__init__(obj=obj, gs=gs)
...
class MTWait(MidTask):
  ...
  def __call__(self, env: EnvState):
    if (self.obj is not None or self.gs
        is not None) and env.navigate_pos_by_obj_gs(self.obj, self.gs):
