...
def bfs_search(grid: List[List[int]], start: Tuple[int, int],
  ...
  while queue:
    ...
    if (curr.x, curr.y) in end:  # wrong when heading multiple end point
      break
    for x_inc, y_inc in [(0, 1), (0, -1), (1, 0),
                         (-1, 0)]:  # wrong when heading multiple end point
      x = curr.x + x_inc
      y = curr.y + y_inc
...
def bfs_reachable(grid: List[List[int]], start: Tuple[int,
                                                      int]) -> List[List[bool]]:
  # used for calculating path for agent
  # input: grid: True for available, False for unavailable
  #        start: start position
...
def match_any(inner: Union[List[str], str, None], outer: Union[List[str], str,
                                                               None]) -> bool:
...
def fname(obj: Union[Object, None]) -> str:
...
def fname_content(obj: Union[Object, None]) -> List[str]:
...
class EnvState:
  ...
  def __init__(self,
    ...
    self.time = time
    # The current game environment. Needed for game simulations.
    self.env = env

    # pos to obj/gs
    self.pos_obj = defaultdict(return_none)
    self.pos_gs = defaultdict(return_none)
    ...
    # all objs
    self.all_obj = [
        a for a in self.world_all if isinstance(a, Object) and not a.is_held
    ]
    self.all_obj_h = self.all_obj + ([self.hold]
                                     if self.hold is not None else [])
    self.all_obj_a = self.all_obj + [
        a.holding for a in self.agents if a.holding is not None
    ]

    # to grid
    self.to_grid = [[1 for _ in range(self.world_height)]
    ...
    for agent in self.agents[:self.agent_idx] + self.agents[self.agent_idx +
                                                            1:]:
    ...
    self.rch_obj = [
        o for o in self.all_obj if self.rch_map[o.location[0]][o.location[1]]
    ]
    self.rch_obj_h = self.rch_obj + ([self.hold]
                                     if self.hold is not None else [])
    self.rch_grid = [
        a for a in self.world_all if isinstance(a, GridSquare)
  ...
  @property
  def hold(self) -> Union[Object, None]:
    # output: object held by agent
    return self.agents[self.agent_idx].holding

  def get_pos_by_obj_gs(self,
                        obj: Union[List[str], str, None] = None,
                        gs: Union[List[str], str, None] = None,
                        inner: bool = False) -> list:
  ...
  def navigate_pos_by_obj_gs(
      ...
      gs: Union[List[str], str, None] = None,
      inner: bool = False,
      agent: bool = False) -> Union[Tuple[int, int], None]:
...
class LTApproach(LowTask):
  ...
  def __call__(self, env: EnvState):
    ...
    other_agent_pos = [
        agent.location
        for agent in env.agents[:env.agent_idx] + env.agents[env.agent_idx + 1:]
    ]
    if distance < 0:
      return LTApproach.DestUnreachable, (0, 0)
    elif move[0] in other_agent_pos:
      return LTApproach.DestBlock, random.choice([(0, 1), (0, -1), (1, 0),
                                                  (-1, 0),
                                                  (0, 0)])  # prevent, stack
    else:
      m = (move[0][0] - env.self_pos[0], move[0][1] - env.self_pos[1])
...
class LTInteract(LowTask):
  ...
  def __call__(self, env: EnvState):
    ...
    if abs(env.self_pos[0] - self.pos[0]) + abs(env.self_pos[1] -
                                                self.pos[1]) > 1:
      return LTInteract.DestTooFar, (0, 0)
    # 2 interact
    return LTInteract.Success, (self.pos[0] - env.self_pos[0],
                                self.pos[1] - env.self_pos[1])
