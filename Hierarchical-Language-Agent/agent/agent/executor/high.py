...
OBJ_TO_GOODS_GS.update({
    ...
    'David Ingredients',
    'ChoppedLettuce-ChoppedOnion-Plate':
    'Alice Ingredients with Plate',
    'ChoppedLettuce-ChoppedTomato-Plate':
    'Bob Ingredients with Plate',
    'ChoppedOnion-ChoppedTomato-Plate':
    'Cathy Ingredients with Plate',
    'ChoppedLettuce-ChoppedOnion-ChoppedTomato-Plate':
    'David Ingredients with Plate',
    ...
    'Plated David Soup',
    'CharredLettuce-CharredOnion-Plate':
    'Plated Charred Alice Soup',
    'CharredLettuce-CharredTomato-Plate':
    'Plated Charred Bob Soup',
    'CharredOnion-CharredTomato-Plate':
    'Plated Charred Cathy Soup',
    'CharredLettuce-CharredOnion-CharredTomato-Plate':
    'Plated Charred David Soup',
})
...
ALL_SALAD = ['Alice Salad', 'Bob Salad', 'Cathy Salad', 'David Salad']
...
class HTClean_(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    # chopped vegetables
    order_fname = [
        a[0].full_name.replace("-Plate", "") for a in env.order.current_orders
    ]
    ...
    for o in env.all_obj_a:
      fns = fname(o).split('-')
      for f in fns:
        f = f.replace("Cooked", "").replace("Cooking",
                                            "").replace("Chopped", "")
        if f in req: req[f] -= 1
    ...
    if len([
        e for e in res
        if e['gs'].name == 'Counter' and e['obj'] is None and e['rch']
    ]) <= 1:
      ...
      elif env.navigate_pos_by_obj_gs(obj=ASSEMBLE_CHARRED_PLATE_FOOD,
                                      gs='Counter') is not None:
      ...
      elif env.navigate_pos_by_obj_gs(obj=USELESS_ASSEMBLES,
                                      gs='Counter') is not None:
        self._task = [MTPick(obj=USELESS_ASSEMBLES, gs='Counter')]
      elif env.navigate_pos_by_obj_gs(obj=ASSEMBLE_CHOPPED_FOOD,
                                      gs='Counter') is not None:
        self._task = [MTPick(obj=ASSEMBLE_CHOPPED_FOOD, gs='Counter')]
      elif env.navigate_pos_by_obj_gs(obj=ASSEMBLE_CHOPPED_PLATE_FOOD,
                                      gs='Counter') is not None:
        self._task = [MTPick(obj=ASSEMBLE_CHOPPED_PLATE_FOOD, gs='Counter')]
      elif env.navigate_pos_by_obj_gs(obj=ASSEMBLE_COOKED_PLATE_FOOD,
                                      gs='Counter') is not None:
...
class HTChop(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    if env.navigate_pos_by_obj_gs(obj='Chopping' + self.ingre,
                                  gs='Cutboard',
                                  agent=True) is None:
      if all([
          fname(env.pos_obj[o.location]) in CHOPPING_FOOD for o in env.rch_grid
          if o.name == 'Cutboard'
      ]):
        if not match_any(fname(env.hold), "Nothing"):
      ...
      if match_any(fname(env.hold), "Nothing"):
        if env.navigate_pos_by_obj_gs(obj='Fresh' + self.ingre,
                                      gs='Counter') is not None:
    ...
    if not match_any(fname(env.hold), "Nothing"):
      ...
      return
    self._task = [
        MTChop(obj='Chopping' + self.ingre),
        # MTPick(obj='Chopped' + self.ingre, gs='Cutboard'),
        # MTPut(gs='Counter')
    ]
    self._last_task = True
    return

  def can_begin(self, env: EnvState, user_reward=False):
    if not user_reward:
      order_fname = [
          a[0].full_name.replace("-Plate", "") for a in env.order.current_orders
      ]
      ...
      order_fname = [
          a[0].full_name.replace("-Plate", "") for a in env.order.current_orders
          if OBJ_TO_GOODS_POT[a[0].full_name.replace("-Plate", "")] in
          env.order.priority_flat
      ]
      # print('order fname: ', order_fname)
    req = {a: 0 for a in ALL_FRESH_FOOD}
    for o in order_fname:
      fns = o.split('-')
      for f in fns:
    ...
    if env.navigate_pos_by_obj_gs(gs="Fresh" + self.ingre + "Tile") is not None:
      can_begin = True
    elif env.navigate_pos_by_obj_gs(obj="Fresh" + self.ingre,
                                    gs='Counter') is not None:
      can_begin = True
    elif env.navigate_pos_by_obj_gs(obj="Chopping" + self.ingre,
                                    gs='Cutboard') is not None:
      can_begin = True
    ...
    if can_begin:
      ...
      return can_begin, f"You cannot chop the {self.ingre}.", [], score
...
class HTAssemble(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    if targ[0] not in rch_obj_fname:
      self._task = [
          MTFail(f"You need to chop {OBJ_TO_GOODS_GS[targ[0]]} first.")
      ]
      return
    if targ[1] is not None and targ[1] not in rch_obj_fname:
      self._task = [
          MTFail(f"You need to chop {OBJ_TO_GOODS_GS[targ[1]]} first.")
      ]
      return
  ...
  def can_begin(self, env: EnvState, user_reward=False):
    ...
    if not user_reward:
      cur_orders = [
          a for a in env.order.current_orders
          if a[0].full_name.replace("-Plate", "") == target
      ]
      multiplier = 1
    else:
      cur_orders = [
          a for a in env.order.current_orders
          if OBJ_TO_GOODS_POT[a[0].full_name.replace("-Plate", "")] in env.
          order.priority_flat and a[0].full_name.replace("-Plate", "") == target
      ]
      multiplier = 1
      if len(cur_orders) == 0:
        cur_orders = [
    ...
    finish_dish = [
        a for a in env.all_obj_a if fname(a) in [
            target,
            target.replace("Cooked", "Cooking"),
            target.replace("Cooked", "Chopped")
        ]
    ]
    cur_orders = cur_orders[len(finish_dish):]
    score = (len(cur_orders) > 0) * 0.52 * multiplier
    ...
    if len(miss) > 0:
      ...
      for idx, m in enumerate(miss[::-1]):
        ...
        if idx == 0:
          ...
          prompt = m + ", " + prompt
      return False, f"You need to chop {prompt} first.", [
          str(HTChop(x.replace('Chopped', ''))) for x in miss
      ], 0
...
class HTPutout(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    if not env.navigate_pos_by_obj_gs(obj='Fire', gs='Pot', inner=True):
      self._task = [
          MTFail(
              "There is currently no fire on the map, so the situation is safe."
          )
      ]
      return
  ...
  def can_begin(self, env: EnvState):
    has_fire = env.navigate_pos_by_obj_gs(obj='Fire', gs='Pot',
                                          inner=True) is not None
    rch_obj_fname = [fname(a) for a in env.rch_obj_h]
...
class HTCook(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    if ingred not in rch_obj_fname and ingred2 not in rch_obj_fname:
      self._task = [
          MTFail(
              f"You should assemble {OBJ_TO_GOODS_GS[ingred]} first before cooking the soup."
          )
      ]

    if all([
        "Fire" in fname(env.pos_obj[o.location]) for o in env.rch_grid
        if o.name == 'Pot'
    ]):
      self._task = [
          MTFail(
              "You need to put out the fire in the pot first because all pots are currently on fire."
          )
      ]
      return
    ...
    if env.navigate_pos_by_obj_gs(obj='Nothing', gs='Pot') is None:
      if not match_any(fname(env.hold), 'Plate'):
        ...
        return
      self._task = [
          MTWait(obj=ASSEMBLE_COOKED_FOOD + ASSEMBLE_CHARRED_FOOD,
                 gs='Pot',
                 timeout=10),
  ...
  def can_begin(self, env: EnvState, user_reward=False):
    ...
    if not user_reward:
      cur_orders = [
          a for a in env.order.current_orders
          if a[0].full_name.replace("-Plate", "") == self.target
      ]
      multiplier = 1
    else:
      cur_orders = [
          a for a in env.order.current_orders
          if OBJ_TO_GOODS_POT[a[0].full_name.replace("-Plate", "")] in env.order
          .priority_flat and a[0].full_name.replace("-Plate", "") == self.target
      ]
      multiplier = 1
      if len(cur_orders) == 0:
        cur_orders = [
    ...
    finish_dish = [
        a for a in env.all_obj_a if fname(a) in [
            GOODS_TO_OBJ_GS[self.soup], GOODS_TO_OBJ_GS[self.soup].replace(
                "Cooked", "Cooking")
        ]
    ]
    cur_orders = cur_orders[len(finish_dish):]
    score = (len(cur_orders) > 0) * 0.54 * multiplier
    ...
    if ingred not in rch_obj_fname and ingred2 not in rch_obj_fname:
      return False, f"You should assemble {OBJ_TO_GOODS_GS[ingred]} first before cooking the soup.", [
          str(HTAssemble(OBJ_TO_GOODS_GS[ingred]))
      ], 0
    ...
    if all([
      ...
      return False, "You need to put out the fire in the pot first because all pots are currently on fire.", [
          HTPutout()
      ], 0
...
class HTPick(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    if self.target not in rch_obj_fname:
      if self.target.replace("Cooked", "Cooking") in rch_obj_fname:
        self._task = [
            MTFail(f"The {self.dish} is cooking, so you cannot pick it up.")
        ]
        return
      else:
        self._task = [
            MTFail(
                f"The {self.dish} is not cooked yet, so you cannot pick it up.")
        ]
        return
  ...
  def can_begin(self, env: EnvState, user_reward=False):
    ...
    cur_orders.sort(key=lambda x: x[1] / x[2])
    finish_dish = [
        a for a in env.all_obj_a if fname(a) == GOODS_TO_OBJ_GS[self.dish]
    ]
    cur_orders = cur_orders[len(finish_dish):]
    if len(cur_orders) == 0:
      score1 = 0
    ...
    match_soup = [
        e for e in grid_info
        if e['gs'].name == 'Pot' and fname(e['obj']) == self.target
    ]
    if len(match_soup) == 0:
      score2 = 0
    else:
      score2 = 1 - min([
          e['obj'].rest_turn_time() / COOKED_BEFORE_FIRE_TIME_SECONDS
          for e in match_soup
      ])

    score = max(score1, score2) * multiplier

    if "Plate" not in rch_obj_fname and env.navigate_pos_by_obj_gs(
        gs='PlateTile') is None:
...
class HTServe(HighTask):
  ...
  def _get_subtask(self, env: EnvState):
    ...
    if targ not in rch_obj_fname:
      if targ.replace("Cooked", "Cooking") in rch_obj_fname:
        ...
        self._task = [
            MTFail(
                f"The {self.dish} is not yet cooked, so you cannot serve it.")
        ]
        return
  ...
  def can_begin(self, env: EnvState, user_reward=False):
    ...
    if len(cur_orders) == 0:
      ...
      score = (1 - min(0.42, min([a[1] / a[2]
                                  for a in cur_orders]))) * multiplier
...
class HTDrop(HighTask):
  ...
  def can_begin(self, env: EnvState):
    ...
    if not match_any(rch_obj_fname,
                     ASSEMBLE_CHARRED_PLATE_FOOD + ASSEMBLE_CHARRED_FOOD):
      if match_any(rch_obj_fname, add_something(ASSEMBLE_CHARRED_FOOD, "Fire")):
        return False, f"The charred food is on fire; you need to put out the fire first.", [
            str(HTPutout())
        ], 0
...
class HTWait(HighTask):
  ...
  def can_begin(self, env: EnvState):
    return True, f"You can wait.", [], 0.5

  def __str__(self):
    return f"{HT_MAP['Wait']}"
