...
def infer_llamacpp(prompt):
  ...
  print('start inference...')
  output = shared.model.generate(prompt, state)
  print('output: ', output)
  return output
...
class Handler(BaseHTTPRequestHandler):
  ...
  def do_POST(self):
    ...
    if self.path == '/api/v1/generate':
      ...
      generator = generate_reply(prompt,
                                 generate_params,
      ...
      response = json.dumps({'results': [{'text': answer}]})
      ...
      generator = generate_chat_reply(user_input,
                                      generate_params,
      ...
      response = json.dumps({'results': [{'history': answer}]})
      ...
      # ret = infer(prompts[0])
      # ret = infer_llamacpp(prompts[0])

      response = json.dumps({'ret': ret})
      ...
      response = json.dumps({'results': 'success'})
      ...
      response = json.dumps({'results': [{'tokens': len(tokens)}]})
