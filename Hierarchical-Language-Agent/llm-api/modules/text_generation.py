...
def _generate_reply(question,
  ...
  if generate_func is None:
    ...
    if shared.model.__class__.__name__ in [
        'LlamaCppModel', 'RWKVModel', 'ExllamaModel', 'CtransformersModel',
        'ExLlamaV2Model'
    ]:
  ...
  all_stop_strings = []
  for st in (stopping_strings,
             ast.literal_eval(f"[{state['custom_stopping_strings']}]")):
...
def encode(prompt,
  ...
  if shared.model.__class__.__name__ in [
      'LlamaCppModel', 'RWKVModel', 'CtransformersModel'
  ]:
    input_ids = shared.tokenizer.encode(str(prompt))
    input_ids = np.array(input_ids).reshape(1, len(input_ids))
  elif shared.model.__class__.__name__ in ['ExLlamaV2Model']:
    input_ids = shared.tokenizer.encode(str(prompt))
...
def generate_reply_wrapper(question, state, stopping_strings=None):
  ...
  for reply in generate_reply(question,
                              state,
                              stopping_strings,
                              is_chat=False,
                              escape_html=True):
...
def get_reply_from_output_ids(output_ids,
                              input_ids,
                              original_question,
                              state,
                              is_chat=False):
  if shared.is_seq2seq:
    ...
    if type(shared.tokenizer) in [
        transformers.LlamaTokenizer, transformers.LlamaTokenizerFast
    ] and len(output_ids) > 0:
      if shared.tokenizer.convert_ids_to_tokens(int(
          output_ids[-new_tokens])).startswith('▁'):
        reply = ' ' + reply
...
def generate_reply_HF(question,
                      original_question,
                      seed,
                      state,
                      stopping_strings=None,
  ...
  generate_params = {}
  for k in [
      'max_new_tokens', 'do_sample', 'temperature', 'top_p', 'typical_p',
      'repetition_penalty', 'repetition_penalty_range',
      'encoder_repetition_penalty', 'top_k', 'min_length',
      'no_repeat_ngram_size', 'num_beams', 'penalty_alpha', 'length_penalty',
      'early_stopping', 'tfs', 'top_a', 'mirostat_mode', 'mirostat_tau',
      'mirostat_eta', 'guidance_scale'
  ]:
  ...
  question, input_ids, inputs_embeds = apply_extensions('tokenizer', state,
                                                        question, input_ids,
                                                        None)
  ...
  eos_token_ids = [shared.tokenizer.eos_token_id
                   ] if shared.tokenizer.eos_token_id is not None else []
  ...
  try:
    ...
    if not state['stream']:
      ...
      yield get_reply_from_output_ids(output,
                                      input_ids,
                                      original_question,
                                      state,
                                      is_chat=is_chat)
...
def generate_reply_custom(question,
                          original_question,
                          seed,
                          state,
                          stopping_strings=None,
