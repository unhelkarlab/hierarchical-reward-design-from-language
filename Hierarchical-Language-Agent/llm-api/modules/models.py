...
if shared.args.deepspeed:
  ...
  # Distributed setup
  local_rank = shared.args.local_rank if shared.args.local_rank is not None else int(
      os.getenv("LOCAL_RANK", "0"))
  world_size = int(os.getenv("WORLD_SIZE", "1"))
  torch.cuda.set_device(local_rank)
  deepspeed.init_distributed()
  ds_config = generate_ds_config(shared.args.bf16, 1 * world_size,
                                 shared.args.nvme_offload_dir)
  dschf = HfDeepSpeedConfig(
      ds_config)  # Keep this object alive for the Transformers integration
  ...
  if tokenizer.__class__.__name__ == 'LlamaTokenizer':
    pairs = [
        [
            ...
            '516c6167c884793a738c440e29ccb80c15e1493ffc965affc69a1a8ddef4572a'
        ],
        [
...
def huggingface_loader(model_name):
  ...
  if not any([
      shared.args.cpu, shared.args.load_in_8bit, shared.args.load_in_4bit,
      shared.args.auto_devices, shared.args.disk, shared.args.deepspeed,
      shared.args.gpu_memory is not None, shared.args.cpu_memory is not None,
      shared.args.compress_pos_emb > 1, shared.args.alpha_value > 1
  ]):
  ...
  # Custom
    ...
    if shared.args.cpu:
      ...
      if shared.args.load_in_4bit:
        ...
        quantization_config_params = {
            ...
            eval("torch.{}".format(shared.args.compute_dtype))
            if shared.args.compute_dtype in ["bfloat16", "float16", "float32"]
            else None,
            'bnb_4bit_quant_type':
            shared.args.quant_type,
            'bnb_4bit_use_double_quant':
            shared.args.use_double_quant,
        }

        logger.warning("Using the following 4-bit params: " +
    ...
    if shared.args.load_in_8bit and params.get(
        'max_memory', None) is not None and params['device_map'] == 'auto':
      config = AutoConfig.from_pretrained(
    ...
    if shared.args.compress_pos_emb > 1:
      ...
      params['rope_scaling'] = {
          ...
          'dynamic',
          'factor':
          RoPE.get_alpha_value(shared.args.alpha_value,
                               shared.args.rope_freq_base)
      }
...
def RWKV_loader(model_name):
  ...
  model = RWKVModel.from_pretrained(
      Path(f'{shared.args.model_dir}/{model_name}'),
      dtype="fp32"
      if shared.args.cpu else "bf16" if shared.args.bf16 else "fp16",
...
def llamacpp_loader(model_name):
  ...
  if path.is_file():
    ...
    model_file = (
        list(Path(f'{shared.args.model_dir}/{model_name}').glob('*.gguf*')) +
        list(Path(f'{shared.args.model_dir}/{model_name}').glob('*ggml*.bin'))
    )[0]
...
def llamacpp_HF_loader(model_name):
  ...
  for fname in ["oobabooga_llama-tokenizer", "llama-tokenizer"]:
    ...
    logger.error(
        "Could not load the model because a tokenizer in transformers format was not found. Please download oobabooga/llama-tokenizer."
    )
    return None, None

  tokenizer = AutoTokenizer.from_pretrained(
      path, trust_remote_code=shared.args.trust_remote_code, use_fast=False)
...
def exllama2_loader(model_name):
  from modules.exllama2_model import ExLlamaV2Model
