...
from modules.utils import (delete_file, get_available_characters, replace_all,
                           save_file)
...
def get_turn_substrings(state, instruct=False):
  ...
  output = {
      'user_turn':
      template.split('<|bot|>')[0],
      'bot_turn':
      '<|bot|>' + template.split('<|bot|>')[1],
      'user_turn_stripped':
      template.split('<|bot|>')[0].split('<|user-message|>')[0],
      'bot_turn_stripped':
      '<|bot|>' + template.split('<|bot|>')[1].split('<|bot-message|>')[0],
  }
...
def generate_chat_prompt(user_input, state, **kwargs):
  ...
  if state['mode'] == 'chat-instruct':
    wrapper = ''
    command = state['chat-instruct_command'].replace(
        '<|character|>', state['name2'] if not impersonate else state['name1'])
    wrapper += state['context_instruct']
    wrapper += all_substrings['instruct']['user_turn'].replace(
    ...
    if impersonate:
      ...
      wrapper += apply_extensions('bot_prefix',
                                  substrings['bot_turn_stripped'].rstrip(' '),
                                  state)
  else:
    wrapper = '<|prompt|>'
  ...
  if is_instruct:
    ...
    context = replace_character_names(f"{state['context'].strip()}\n",
  ...
  min_rows = 3
  i = len(history) - 1
  while i >= 0 and get_encoded_length(
      wrapper.replace('<|prompt|>', ''.join(rows))) < max_length:
    if _continue and i == len(history) - 1:
      if state['mode'] != 'chat-instruct':
        rows.insert(1, substrings['bot_turn_stripped'] + history[i][1].strip())
    else:
      rows.insert(
          1, substrings['bot_turn'].replace('<|bot-message|>',
                                            history[i][1].strip()))
    ...
    if string not in ['', '<|BEGIN-VISIBLE-CHAT|>']:
      rows.insert(
          1,
          replace_all(substrings['user_turn'], {
              '<|user-message|>': string.strip(),
              '<|round|>': str(i)
          }))
  ...
  if impersonate:
    ...
    if len(user_input) > 0:
      rows.append(
          replace_all(
              substrings['user_turn'], {
                  '<|user-message|>': user_input.strip(),
                  '<|round|>': str(len(history))
              }))

    # Add the character prefix
    if state['mode'] != 'chat-instruct':
      rows.append(
          apply_extensions('bot_prefix',
                           substrings['bot_turn_stripped'].rstrip(' '), state))

  while len(rows) > min_rows and get_encoded_length(
      wrapper.replace('<|prompt|>', ''.join(rows))) >= max_length:
    rows.pop(1)
...
def get_stopping_strings(state):
  ...
  if state['mode'] in ['instruct', 'chat-instruct']:
    ...
    for i in range(len(stopping_strings)):
      stopping_strings[i] = replace_all(stopping_strings[i],
                                        replacements).rstrip(' ').replace(
                                            r'\n', '\n')

  if state['mode'] in ['chat', 'chat-instruct']:
    stopping_strings += [f"\n{state['name1']}:", f"\n{state['name2']}:"]

  if 'stopping_strings' in state and isinstance(state['stopping_strings'],
                                                list):
...
def chatbot_wrapper(text,
  ...
  if not any((regenerate, _continue)):
    ...
    # Apply extensions
    text, visible_text = apply_extensions('chat_input', text, visible_text,
                                          state)
    ...
    if loading_message:
      yield {
          'visible':
          output['visible'] + [[visible_text, shared.processing_message]],
    ...
    if regenerate:
      ...
      if loading_message:
        yield {
            'visible':
            output['visible'][:-1] + [[visible_text, last_reply[1] + '...']],
            'internal':
            output['internal']
        }
  ...
  prompt = apply_extensions('custom_generate_chat_prompt', text, state,
                            **kwargs)
  if prompt is None:
  ...
  for j, reply in enumerate(
    ...
    if shared.stop_everything:
      output['visible'][-1][1] = apply_extensions('output',
                                                  output['visible'][-1][1],
                                                  state,
                                                  is_chat=True)
      yield output
...
def impersonate_wrapper(text, state):
  ...
  yield text + '...'
  reply = None
  for reply in generate_reply(prompt + text,
                              state,
                              stopping_strings=stopping_strings,
                              is_chat=True):
...
def generate_chat_reply(text,
  ...
  if regenerate or _continue:
    text = ''
    if (len(history['visible']) == 1 and not history['visible'][0][0]) or len(
        history['internal']) == 0:
      yield history
      return
...
def generate_chat_reply_wrapper(text, state, regenerate=False, _continue=False):
  ...
  for i, history in enumerate(
      generate_chat_reply(text,
                          state,
                          regenerate,
                          _continue,
                          loading_message=True)):
    yield chat_html_wrapper(history, state['name1'], state['name2'],
                            state['mode'], state['chat_style']), history


def remove_last_message(history):
  if len(history['visible']
         ) > 0 and history['internal'][-1][0] != '<|BEGIN-VISIBLE-CHAT|>':
...
def replace_last_reply(text, state):
  ...
  if len(text.strip()) == 0:
    ...
    history['internal'][-1][1] = apply_extensions('input',
                                                  text,
                                                  state,
                                                  is_chat=True)
...
def send_dummy_message(text, state):
  ...
  history['internal'].append(
      [apply_extensions('input', text, state, is_chat=True), ''])
  return history
...
def clear_chat_log(state):
  greeting = replace_character_names(state['greeting'], state['name1'],
                                     state['name2'])
  mode = state['mode']
  history = state['history']
  ...
  if mode != 'instruct':
    if greeting != '':
      ...
      history['visible'] += [[
          '', apply_extensions('output', greeting, state, is_chat=True)
      ]]
...
def save_persistent_history(history, character, mode):
  if mode in ['chat', 'chat-instruct'] and character not in [
      '', 'None', None
  ] and not shared.args.multi_user:
...
def load_persistent_history(state):
  ...
  should_load_history = (not shared.args.multi_user
                         and character not in ['None', '', None])
...
def load_character(character, name1, name2, instruct=False):
  ...
  if character not in ['None', '', None]:
    ...
    if filepath is None:
      logger.error(
          f"Could not find character file for {character} in {folder} folder. Please check your spelling."
      )
      return name1, name2, picture, greeting, context, turn_template.replace(
          "\n", r"\n")

    file_contents = open(filepath, 'r', encoding='utf-8').read()
    data = json.loads(file_contents) if extension == "json" else yaml.safe_load(
        file_contents)
...
def upload_character(file, img, tavern=False):
  ...
  if 'char_name' in data:
    ...
    name = data['name']
    yaml_data = generate_character_yaml(data['name'], data['greeting'],
                                        data['context'])
  ...
  with open(Path(f'characters/{outfile_name}.yaml'), 'w',
            encoding='utf-8') as f:
    f.write(yaml_data)
...
def check_tavern_character(img):
  ...
  decoded_string = base64.b64decode(img.info['chara']).replace(
      b'\\r\\n', b'\\n')
