...
class MapSetting:
  ...
  priority: list = field(default_factory=list)  # list of orders to prioritize
...
class OvercookedEnvironment(gym.Env):
  ...
  def __init__(self, arglist, seed=0):
    super().__init__()
  ...
  def load_level(self, level, num_agents):
    ...
    with open(
        os.path.join(
            Path(gym_cooking.__file__).absolute().parent, 'utils', 'levels',
            '{}.txt'.format(level)), 'r') as file:
      ...
      for line in file:
        ...
        # Phase 3: Read in agent locations (up to num_agents).
          ...
          if len(self.sim_agents) < num_agents:
            loc = line.split(' ')
            # loc = (self.rng.randint(1, 3), self.rng.randint(1, 2))
            sim_agent = SimAgent(name='agent-' + str(len(self.sim_agents) + 1),
    ...
    if self.chg_grid is not None:
      ...
      self.chg_rand_list = [
          random.randint(0,
                         len(self.chg_pos) - 1) for _ in range(101)
      ]
      self.process_chg()
  ...
  def reset(self):
    ...
    self.num_ingredients_chopped = np.zeros(len(Ingredients) - 1)
    self.num_ingredients_combined = np.zeros(len(SoupType) - 1)
    self.num_soup_plated = np.zeros(len(SoupType) - 1)
    self.num_fires = 0
    self.reward_sum = 0
    self.recipes = []
    self.sim_agents = []
    ...
    self.order_scheduler = OrderScheduler(self.arglist,
                                          self.recipes,
  ...
  def step(self, action_dict, passed_time=1.):
    ...
    for event in events:
      ...
      if event.event not in self.all_events and not 'No-op_' in event.event:
    ...
    r_sum, p_sum = self.reward()
    reward_sum = r_sum + p_sum
    reward = reward_sum - self.reward_sum
  ...
  def reward(self):
    ...
    reward = self.order_scheduler.reward
    p_reward = self.order_scheduler.p_reward
    return reward, p_reward
  ...
  def update_object_states(self):
    ...
    for pot in self.world.get_all_gridsquares('Pot'):
      if pot.holding is not None and (
          pot.holding.is_cooking()
          or pot.holding.is_cooked()) and 'Fire' in pot.holding.full_name:
        fire = [x for x in pot.holding.contents if isinstance(x, Fire)][0]
        for agent in self.sim_agents:
          if agent.action == (0, 0):
            continue
  ...
  def process_chg(self):
    ...
    self.chg_rand_index = (self.chg_rand_index + 1) % len(self.chg_rand_list)
    ...
    for source in source_objs:
      ...
      for destination in dest_objs:
        ...
        for source_edge, dest_edge in product(source_edges, destination_edges):
          try:
            dist = nx.shortest_path_length(self.world.reachability_graph,
  ...
  def get_current_state(self):
    # gridsquare
    gridsquare_map = {
        gs: np.zeros(self.world_size, dtype=np.uint8)
    ...
    # objects
    ALL_ENTITIES = FRESH_FOOD + CHOPPING_FOOD + CHOPPED_FOOD + COOKING_FOOD + COOKED_FOOD + CHARRED_FOOD + [
        'Plate', 'Fire', 'FireExtinguisher'
    ]
    ALL_FOOD_ENTITIES = FRESH_FOOD + CHOPPED_FOOD + \
        COOKED_FOOD + CHARRED_FOOD + ['Plate']
    obj_map = {
        e: np.zeros(self.world_size, dtype=np.uint8)
        for e in ALL_ENTITIES
    }
    for o in objs:
      for c in o.contents:
        obj_map[c.full_name][o.location[0], o.location[1]] = 1
    obj_map['cooking_rest_time'] = np.zeros(self.world_size, dtype=np.float32)
    for pot in self.world.get_all_gridsquares('Pot'):
      if pot.holding is not None and pot.holding.is_cooking():
        obj_map['cooking_rest_time'][
            pot.location[0], pot.location[1]] = pot.holding.rest_cooking_time(
            ) / COOKING_TIME_SECONDS
    obj_map['chopping_num_steps'] = np.zeros(self.world_size, dtype=np.float32)
    for cutboard in self.world.get_all_gridsquares('Cutboard'):
      if cutboard.holding is not None and cutboard.holding.full_name.startswith(
          'Chopping'):
        obj_map['chopping_num_steps'][
            cutboard.location[0],
            cutboard.location[1]] = cutboard.holding.contents[
                0].state._rest_steps / CHOPPING_NUM_STEPS

    # agents
    agent_map = {
        agent.name: np.zeros(self.world_size, dtype=np.uint8)
        for agent in self.sim_agents
    }
    for agent in self.sim_agents:
    ...
    for agent in self.sim_agents:
      if agent.holding is None:
        ...
        agent_data[agent.name]['holding_onehot'] = current_holdings[
            agent.name] = np.zeros((len(ALL_FOOD_ENTITIES), ), dtype=np.uint8)
      else:
    ...
    for order, restTime, timeLimit, bonus in self.order_scheduler.current_orders:
      order_onehot = np.array(
          [int(e in order.full_name) for e in ALL_FOOD_ENTITIES])
      current_orders.append((order_onehot, restTime / timeLimit))
    current_orders_np = np.concatenate([
        np.concatenate([order_onehot, np.array([t])])
        for order_onehot, t in current_orders
    ])
    '''result = {
    ...
    original_map = [v for k, v in gridsquare_map.items()] + [
        v for k, v in obj_map.items()
    ] + [v for k, v in agent_map.items()]
    # original_map = [v for k, v in obj_map.items()
    #                 ] + [v for k, v in agent_map.items()]
    original_map = np.array(original_map).transpose((2, 1, 0))
    flat_map = original_map.flatten()
    result = {
        'original_map': original_map,
        'map': np.asarray(flat_map),
    ...
    for event in self.last_step_events:
      if 'No-op_' in event.event:
        chopped_ingre = event.event.split('_')[1]
        if 'Tomato' in chopped_ingre:
          self.num_ingredients_chopped[Ingredients.tomato.value - 1] += 1
          result['just_chopped'] = Ingredients.tomato.value
        elif 'Onion' in chopped_ingre:
          self.num_ingredients_chopped[Ingredients.onion.value - 1] += 1
          result['just_chopped'] = Ingredients.onion.value
        elif 'Lettuce' in chopped_ingre:
          self.num_ingredients_chopped[Ingredients.lettuce.value - 1] += 1
          result['just_chopped'] = Ingredients.lettuce.value
      elif event.event == 'Assemble_ChoppedLettuce-ChoppedOnion':
        result['just_combined'] = SoupType.alice.value
        self.num_ingredients_combined[SoupType.alice.value - 1] += 1
      elif event.event == 'Assemble_ChoppedLettuce-ChoppedTomato':
        result['just_combined'] = SoupType.bob.value
        self.num_ingredients_combined[SoupType.bob.value - 1] += 1
      elif event.event == 'Assemble_ChoppedOnion-ChoppedTomato':
        result['just_combined'] = SoupType.cathy.value
        self.num_ingredients_combined[SoupType.cathy.value - 1] += 1
      elif event.event == 'Assemble_ChoppedLettuce-ChoppedOnion-ChoppedTomato':
        result['just_combined'] = SoupType.david.value
        self.num_ingredients_combined[SoupType.david.value - 1] += 1
      elif event.event == 'Assemble_ChoppedLettuce-ChoppedOnion-Plate':
        result['just_plated'] = SoupType.alice.value
        self.num_soup_plated[SoupType.alice.value - 1] += 1
      elif event.event == 'Assemble_ChoppedLettuce-ChoppedTomato-Plate':
        result['just_plated'] = SoupType.bob.value
        self.num_soup_plated[SoupType.bob.value - 1] += 1
      elif event.event == 'Assemble_ChoppedOnion-ChoppedTomato-Plate':
        result['just_plated'] = SoupType.cathy.value
        self.num_soup_plated[SoupType.cathy.value - 1] += 1
      elif event.event == 'Assemble_ChoppedLettuce-ChoppedOnion-ChoppedTomato-Plate':
        result['just_plated'] = SoupType.david.value
        self.num_soup_plated[SoupType.david.value - 1] += 1
    result['ingre_chopped'] = self.num_ingredients_chopped
    result['ingre_combined'] = self.num_ingredients_combined
    result['soup_plated'] = self.num_soup_plated
    return result
  ...
  def get_current_features(self):
    ...
    for o_list in self.world.objects.values():
      for o in o_list:
        if not isinstance(o, GridSquare):
          objs.append(o)
        else:
          if isinstance(o, Pot):
            num_pots += 1
    ...
    num_of_cooked_dishes = 0
    features = np.zeros(len(FOOD_ENTITIES) + num_pots)
    for o in objs:
      if o.full_name in FOOD_ENTITIES:
        idx = FOOD_ENTITIES.index(o.full_name)
        features[idx] += 1
        if 'Cooked' in o.full_name and 'Plate' not in o.full_name:
          features[num_of_cooked_dishes +
                   len(FOOD_ENTITIES
                       )] = o.rest_turn_time() / COOKED_BEFORE_FIRE_TIME_SECONDS
          num_of_cooked_dishes += 1
    ...
    # print('current orders array: ', current_orders_np)
    # print('concat features: ', np.concatenate((features, current_orders_np)))

    return np.concatenate((features, current_orders_np))
