...
def interact(agent, world, current_time) -> Event:
  ...
  # if holding something
    ...
    if isinstance(gs, Delivery):
      ...
      if obj.is_deliverable():
        ...
        return Event(playerA=agent.name,
                     event=f'Deliver_{obj.full_name}',
    ...
    # if food/plate tile in front --> get food/plate
      ...
      if mergeable(agent.holding, obj):
        ...
        world.insert(agent.holding)
        return Event(playerA=agent.name,
                     event=f'Assemble_{agent.holding.full_name}',
                     location=gs.location,
                     time=current_time)
      # if hold a plate before plate tile --> put the plate into the tile
        ...
        agent.release()
        return Event(playerA=agent.name,
                     event=f'Put_Plate_on_PlateTile',
                     location=gs.location,
                     time=current_time)
    ...
    # if gs is Pot --> try cooking
      ...
      if not world.is_occupied(gs.location):
        ...
        if agent.holding.is_cookable():
          ...
          event = Event(playerA=agent.name,
                        event=f'Cook_{obj_food.full_name}',
                        location=gs.location,
                        time=current_time)
          gs.acquire(obj_food)
        ...
        if agent.holding.contents[0] == Plate() and len(
            agent.holding.contents) == 1:
          ...
          if obj.is_cooked() and not obj.is_onfire():
            ...
            return Event(playerA=agent.name,
                         event=f'Pickup_{obj.full_name}_from_{gs.name}',
                         location=gs.location,
                         time=current_time)
        if agent.holding.contents[0] == FireExtinguisher() and len(
            agent.holding.contents) == 1:
    ...
    # if occupied gridsquare in front --> try merging
      ...
      if mergeable(agent.holding, obj):
        ...
        if agent_has_plate:
          plating_event = Event(playerA=agent.name,
                                event=f'Assemble_{merged_name}',
                                location=gs.location,
                                time=current_time)
          # print('plating_event: ', plating_event.event)
          return plating_event
        else:
          gs.acquire(agent.holding)
          agent.release()
          merging_event = Event(playerA=agent.name,
                                event=f'Assemble_{merged_name}',
                                location=gs.location,
                                time=current_time)
          # print('merging event: ', merging_event.event)
          return merging_event
    ...
    # if holding something, empty gridsquare in front --> chop or drop
      ...
      if event is None:
        event = Event(playerA=agent.name,
                      event=f'Put_{obj.full_name}_on_{gs.name}',
                      location=gs.location,
                      time=current_time)
      agent.release()
  ...
  # if not holding anything
    ...
    elif world.is_occupied(gs.location) and not isinstance(
        gs, Delivery) and not isinstance(gs, Bin):
      obj = world.get_object_at(gs.location, None, find_held_objects=False)
