...
class World:
      ...
      for y in range(self.height):
        ...
        for nav_action in World.NAV_ACTIONS:
          ...
          # If not collidable and new_gs collidable, add edge.
            ...
            if (new_location,
                tuple(-np.asarray(nav_action))) in self.reachability_graph:
  ...
  def get_lower_bound_between(self, subtask, agent_locs, A_locs, B_locs):
    ...
    for A_loc, B_loc in product(A_locs, B_locs):
      bound = self.get_lower_bound_between_helper(subtask=subtask,
  ...
  def get_lower_bound_between_helper(self, subtask, agent_locs, A_loc, B_loc):
    ...
    for A_na, B_na in product(A_possible_na, B_possible_na):
      if len(agent_locs) == 1:
        try:
          bound_1 = nx.shortest_path_length(self.reachability_graph,
                                            (agent_locs[0], (0, 0)),
                                            (A_loc, A_na))
          bound_2 = nx.shortest_path_length(self.reachability_graph,
                                            (A_loc, A_na), (B_loc, B_na))
        except:
          continue
        ...
        try:
          bound_1_to_A = nx.shortest_path_length(self.reachability_graph,
                                                 (agent_locs[0], (0, 0)),
                                                 (A_loc, A_na))
        except:
        ...
        try:
          bound_2_to_A = nx.shortest_path_length(self.reachability_graph,
                                                 (agent_locs[1], (0, 0)),
                                                 (A_loc, A_na))
        except:
        ...
        bound_between_agents = float(
            abs(A_loc[0] - B_loc[0]) + abs(A_loc[1] - B_loc[1]))

        # Distance between Agent 1 <> Object B.
        try:
          bound_1_to_B = nx.shortest_path_length(self.reachability_graph,
                                                 (agent_locs[0], (0, 0)),
                                                 (B_loc, B_na))
        except:
        ...
        try:
          bound_2_to_B = nx.shortest_path_length(self.reachability_graph,
                                                 (agent_locs[1], (0, 0)),
                                                 (B_loc, B_na))
        except:
        ...
        # For merge, agents can separately go to A and B and then meet in the middle.
          ...
          bound = max(min_bound_to_A,
                      min_bound_to_B) + (bound_between_agents - 1) / 2
  ...
  def check_bound(self, min_bound_to_A, min_bound_to_B, bound_1_to_A,
                  bound_2_to_A, bound_1_to_B, bound_2_to_B):
    # Checking for whether it's the same agent that does the subtask.
    if ((bound_1_to_A == min_bound_to_A and bound_1_to_B == min_bound_to_B)
        or (bound_2_to_A == min_bound_to_A and bound_2_to_B == min_bound_to_B)):
  ...
  def is_occupied(self, location):
    o = list(
        filter(
            lambda obj: obj.location == location and isinstance(obj, Object) and
            not (obj.is_held), self.get_object_list()))
    if o:
      return True
    return False
  ...
  def remove(self, obj):
    ...
    ) < num_objs, "Nothing from {} was removed from world.objects".format(
        obj.name)
  ...
  def get_dynamic_objects(self):
    ...
    for key in sorted(self.objects.keys()):
      if key != "Counter" and key != "Floor" and "Supply" not in key and key != "Delivery" and key != "Cutboard" and key != "Bin" and "Tile" not in key:
    ...
    if isinstance(obj, Object):
      return list(
          map(
              lambda o: o.location,
              list(
                  filter(lambda o: obj == o and o.is_held == is_held,
                         self.objects[obj.name]))))
    else:
      return list(
          map(lambda o: o.location,
  ...
  def get_object_at(self, location, desired_obj, find_held_objects):
    ...
    if desired_obj is None:
      objs = list(
          filter(
              lambda obj: obj.location == location and isinstance(obj, Object)
              and obj.is_held is find_held_objects, all_objs))
    else:
      objs = list(
          filter(
              lambda obj: obj.name ==
              desired_obj.name and obj.location == location and isinstance(
                  obj, Object) and obj.is_held is find_held_objects, all_objs))
  ...
  def get_gridsquare_at(self, location):
    ...
    assert len(gss) == 1, "{} gridsquares at {}: {}".format(
        len(gss), location, gss)
    return gss[0]
  ...
  def get_all_delivery_gridsquares(self):
    return list(self.objects.get('Delivery', copy.copy([])))

  def get_all_gridsquares(self, gridsquare_name):
    return list(self.objects.get(gridsquare_name, copy.copy([])))
