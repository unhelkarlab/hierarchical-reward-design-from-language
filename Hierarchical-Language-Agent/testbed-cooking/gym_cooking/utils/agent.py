...
class SimAgent:
  ...
  def get_repr(self):
    return AgentRepr(name=self.name,
  ...
  def print_status(self):
    print("{} currently at {}, action {}, holding {}".format(
        color(self.name, self.color), self.location, self.action,
        self.get_holding()))
  ...
  def set_cur_intent(self, intent):
    self.intent = intent

  def get_intent_hist(self):
    return self.intent_hist

  def set_intent_hist(self, intent_hist: List[str]):
    self.intent_hist = intent_hist
