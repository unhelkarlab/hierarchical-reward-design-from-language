...
ValidFoodNames = ('ChoppedLettuce', 'ChoppedOnion', 'ChoppedTomato',
                  'ChoppedLettuce-ChoppedOnion', 'ChoppedLettuce-ChoppedTomato',
...
class Object:
  ...
  def __str__(self):
    res = "-".join(
        list(map(lambda x: str(x), sorted(self.contents,
                                          key=lambda i: i.name))))
    return res
  ...
  def get_repr(self):
    return ObjectRepr(name=self.full_name,
  ...
  def is_deliverable(self):
    ...
    for c in self.contents:
      if not (isinstance(c, Plate) or
              (isinstance(c, Food) and c.is_deliverable())):
        return False
...
class FoodSequence:
  ...
  FRESH_CHOPPING_CHOPPED = [
      FoodState.FRESH, FoodState.CHOPPING, FoodState.CHOPPED
  ]
  FRESH_CHOPPING_CHOPPED_COOKING_COOKED = FRESH_CHOPPING_CHOPPED + [
      FoodState.COOKING, FoodState.COOKED
  ]
  FRESH_CHOPPING_CHOPPED_COOKING_COOKED_CHARRED = FRESH_CHOPPING_CHOPPED_COOKING_COOKED + [
      FoodState.CHARRED
  ]
...
class Food:
  ...
  def set_state(self, state):
    assert state in self.state_seq, "Desired state {} does not exist for the food with sequence {}".format(
  ...
  def needs_chopped(self):
    return self.state_seq[(self.state_index + 1) % len(self.state_seq)] in [
        FoodState.CHOPPED, FoodState.CHOPPING
    ]
  ...
  def update_state(self, current_time):
    if isinstance(self.state, FoodState.COOKING) or isinstance(
        self.state, FoodState.COOKED):
    ...
    if self.state.is_finished:
      ...
      assert 0 <= self.state_index and self.state_index < len(
          self.state_seq), "State index is out of bounds for its state sequence"
      self.state = self.state_seq[self.state_index](obj=self.name,
  ...
  # cook logic
  def is_cookable(self):
    return FoodState.COOKING in self.state_seq and 0 <= self.state_index and self.state_index < len(
        self.state_seq) and self.state_seq[
            (self.state_index + 1) % len(self.state_seq)] == FoodState.COOKING

  def is_cooked(self):
    return isinstance(self.state, FoodState.COOKED) or isinstance(
        self.state, FoodState.CHARRED)
...
class Fire:
  ...
  def update_state(self, current_time):
    ...
    last_putout_time = max([v
                            for k, v in self.last_putout_time.items()] + [-1e9])
    decrease = max([
        0,
        min([current_time, last_putout_time + FIRE_RECOVER_GAP_TIME_SECONDS]) -
        max([self.latest_time, last_putout_time])
    ])
    increase = max([
        0, current_time - max([
            self.latest_time, last_putout_time + FIRE_RECOVER_GAP_TIME_SECONDS
        ])
    ])
...
GRIDSQUARES = [
    "Floor", "Counter", "Cutboard", "Delivery", "Bin", "Pot", "FreshTomatoTile",
...
ASSEMBLE_CHOPPED_FOOD = [
    'ChoppedTomato', 'ChoppedOnion', 'ChoppedLettuce',
    'ChoppedLettuce-ChoppedOnion', 'ChoppedLettuce-ChoppedTomato',
    'ChoppedOnion-ChoppedTomato', 'ChoppedLettuce-ChoppedOnion-ChoppedTomato'
]
ASSEMBLE_CHOPPED_PLATE_FOOD = add_something(ASSEMBLE_CHOPPED_FOOD, 'Plate')
ASSEMBLE_COOKING_FOOD = [
    f.replace('Chopped', 'Cooking') for f in ASSEMBLE_CHOPPED_FOOD
]
ASSEMBLE_COOKING_PLATE_FOOD = add_something(ASSEMBLE_COOKING_FOOD, 'Plate')
ASSEMBLE_COOKED_FOOD = [
    f.replace('Chopped', 'Cooked') for f in ASSEMBLE_CHOPPED_FOOD
]
ASSEMBLE_COOKED_PLATE_FOOD = add_something(ASSEMBLE_COOKED_FOOD, 'Plate')
ASSEMBLE_CHARRED_FOOD = [
    f.replace('Chopped', 'Charred') for f in ASSEMBLE_CHOPPED_FOOD
]
