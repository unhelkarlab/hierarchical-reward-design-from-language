...
class Predicate():
  ...
  def __init__(self, name, args, time_limit=1, start_time=None):
    self.name = name
    self.args = args
    self._rest_steps = time_limit
  ...
  def is_finished(self):
    return (self._rest_steps <= 0)

  def update_one_step(self, passed=1):
    self._rest_steps -= passed

  def update_by_current_time(self, current_time=None):
    assert current_time and self._start_time
    self._rest_steps = self._time_limit - (current_time - self._start_time)

  def recover_one_step(self, t):
    self._rest_steps = self._rest_steps + t
    if self._rest_steps >= self._time_limit:
      self._rest_steps = self._time_limit
...
class Cooking(Predicate):

  def __init__(self, obj, start_time=None):
    Predicate.__init__(self,
                       'Cooking', (obj, ),
                       start_time=start_time,
                       time_limit=COOKING_TIME_SECONDS)
    self._rest_steps = COOKING_TIME_SECONDS
...
class Cooked(Predicate):
  ...
  def __init__(self, obj, start_time=None):
    Predicate.__init__(self,
                       'Cooked', (obj, ),
                       start_time=start_time,
                       time_limit=COOKED_BEFORE_FIRE_TIME_SECONDS)
...
class Charred(Predicate):

  def __init__(self, obj, **kwargs):
    Predicate.__init__(self, 'Charred', (obj, ))
...
class STRIPSState:
  ...
  def __eq__(self, other):
    if other is None:
      return False
    return sorted([str(p) for p in self.predicates
                   ]) == sorted([str(p) for p in other.predicates])
  ...
  def delete_predicate(self, predicate):
    assert predicate in self.predicates, "{} not in this state".format(
        predicate)
...
def make_predicate_graph(initial, action_path, draw=True):
  ...
  for predicate in initial.predicates:
    ...
    node_labels[predicate] = 'START' if predicate == NoPredicate() else str(
        predicate)
...
def make_graph(g, node_labels, edge_labels, draw):
  ...
  labels_layout = dict(
      map(lambda kv: (kv[0], kv[1] + np.asarray([0, -0.1])), layout.items()))
